...
def get_index():
    try:
        return cached("index", 15, lambda: fetch_quote(NIFTY_INDEX, name="NIFTY 50"))
...
def get_quote(symbol: str):
    symbol = symbol.upper()
    try:
        return cached(f"quote:{symbol}", 15, lambda: fetch_quote(symbol))
    except Exception as e:
        raise HTTPException(502, f"Failed to fetch {symbol}: {e}")


def fetch_quotes_batch(tickers: list) -> list:
    """Fetch quotes for several tickers in one request instead of one-by-one —
    much faster and far less likely to trip Yahoo's rate limits."""
    ...
    for t in tickers:
        try:
            ...
            results.append({
                ...
                "day_high": round(float(highs.iloc[-1]), 2) if len(highs) else current,
                "day_low": round(float(lows.iloc[-1]), 2) if len(lows) else current,
...
def get_watchlist():
    def fetch():
        items = fetch_quotes_batch(WATCHLIST_DEFAULT)
        return {"items": items, "updated": int(time.time())}

    try:
        return cached("watchlist", 20, fetch)
    except Exception as e:
...
def get_history(symbol: str, range: str = "1M"):
    ...
    try:
        return cached(f"hist:{symbol}:{range}", 30, fetch)
    except Exception as e:
